...
clearcart.addEventListener("click", function (e) {
    ...
    $.ajax({
        ...
        success: function(data) {
            document.querySelector(".cart-goods").parentElement.innerHTML = '<h1 align="center">Корзина пуста</h1>';
            allprice();
        }
    });
});
...
plus.forEach(function (p) {
    p.addEventListener("click", function(){
        ...
        let price = document.querySelector("#good"+plusid+" > h5 > .span-price").innerHTML;
        $.ajax({
            type: "POST",
            url: "/includes/plus.php",
            data: "plusid="+plusid,
            success: function(data) {
                document.querySelector("#good"+plusid+" > h5 > .span-count").innerHTML = data;
...
minus.forEach(function (m) {
    m.addEventListener("click", function(){
        ...
        let price = document.querySelector("#good"+minusid+" > h5 > .span-price").innerHTML;
        $.ajax({
            type: "POST",
            url: "/includes/minus.php",
            data: "minusid="+minusid,
            success: function(data) {
                document.querySelector("#good"+minusid+" > h5 > .span-count").innerHTML = data;
